...
ggplot(topfrequentobst_nw, aes(y = Freq, axis1 = obstacle_name ,  axis3 = round_stage, axis2 = season )) +
  geom_alluvium(aes(fill = obstacle_name), width = 1/12) +
  theme_bw() +
  geom_stratum(width = 1/12, fill = "gray27", color = "grey") +
  geom_label(stat = "stratum", aes(label = after_stat(stratum))) +
  scale_x_discrete(limits = c("Obstacle", "Round stage" , "Season"), expand = c(.05, .05)) +
